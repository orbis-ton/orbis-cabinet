...
export type ExchangerData = {
    ...
    myJettonWalletOld: Address | null;
    collection: Address;
    collectionOld: Address;
    balance: bigint | null;
    balanceOld: bigint | null;
    maxItemIndex: bigint;
    expectedNftItem0Address: Address;
}

export function storeExchangerData(src: ExchangerData) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeAddress(src.myJettonWallet);
        b_0.storeAddress(src.myJettonWalletOld);
        b_0.storeAddress(src.collection);
        const b_1 = new Builder();
        b_1.storeAddress(src.collectionOld);
        if (src.balance !== null && src.balance !== undefined) { b_1.storeBit(true).storeCoins(src.balance); } else { b_1.storeBit(false); }
        if (src.balanceOld !== null && src.balanceOld !== undefined) { b_1.storeBit(true).storeCoins(src.balanceOld); } else { b_1.storeBit(false); }
        b_1.storeInt(src.maxItemIndex, 257);
        const b_2 = new Builder();
        b_2.storeAddress(src.expectedNftItem0Address);
        b_1.storeRef(b_2.endCell());
        b_0.storeRef(b_1.endCell());
    };
}

export function loadExchangerData(slice: Slice) {
    const sc_0 = slice;
    const _myJettonWallet = sc_0.loadMaybeAddress();
    const _myJettonWalletOld = sc_0.loadMaybeAddress();
    const _collection = sc_0.loadAddress();
    const sc_1 = sc_0.loadRef().beginParse();
    const _collectionOld = sc_1.loadAddress();
    const _balance = sc_1.loadBit() ? sc_1.loadCoins() : null;
    const _balanceOld = sc_1.loadBit() ? sc_1.loadCoins() : null;
    const _maxItemIndex = sc_1.loadIntBig(257);
    const sc_2 = sc_1.loadRef().beginParse();
    const _expectedNftItem0Address = sc_2.loadAddress();
    return { $$type: 'ExchangerData' as const, myJettonWallet: _myJettonWallet, myJettonWalletOld: _myJettonWalletOld, collection: _collection, collectionOld: _collectionOld, balance: _balance, balanceOld: _balanceOld, maxItemIndex: _maxItemIndex, expectedNftItem0Address: _expectedNftItem0Address };
}

function loadTupleExchangerData(source: TupleReader) {
    const _myJettonWallet = source.readAddressOpt();
    const _myJettonWalletOld = source.readAddressOpt();
    const _collection = source.readAddress();
    const _collectionOld = source.readAddress();
    const _balance = source.readBigNumberOpt();
    const _balanceOld = source.readBigNumberOpt();
    const _maxItemIndex = source.readBigNumber();
    const _expectedNftItem0Address = source.readAddress();
    return { $$type: 'ExchangerData' as const, myJettonWallet: _myJettonWallet, myJettonWalletOld: _myJettonWalletOld, collection: _collection, collectionOld: _collectionOld, balance: _balance, balanceOld: _balanceOld, maxItemIndex: _maxItemIndex, expectedNftItem0Address: _expectedNftItem0Address };
}

function loadGetterTupleExchangerData(source: TupleReader) {
    const _myJettonWallet = source.readAddressOpt();
    const _myJettonWalletOld = source.readAddressOpt();
    const _collection = source.readAddress();
    const _collectionOld = source.readAddress();
...
function storeTupleExchangerData(source: ExchangerData) {
    const builder = new TupleBuilder();
    builder.writeAddress(source.myJettonWallet);
    builder.writeAddress(source.myJettonWalletOld);
    builder.writeAddress(source.collection);
    builder.writeAddress(source.collectionOld);
    builder.writeNumber(source.balance);
    builder.writeNumber(source.balanceOld);
    builder.writeNumber(source.maxItemIndex);
    builder.writeAddress(source.expectedNftItem0Address);
    return builder.build();
}

function dictValueParserExchangerData(): DictionaryValue<ExchangerData> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeExchangerData(src)).endCell());
        },
        parse: (src) => {
            return loadExchangerData(src.loadRef().beginParse());
        }
    }
}

export type Exchanger$Data = {
    $$type: 'Exchanger$Data';
    initialized: boolean;
    myJettonWallet: Address | null;
    myJettonWalletOld: Address | null;
...
export function storeExchanger$Data(src: Exchanger$Data) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeBit(src.initialized);
        b_0.storeAddress(src.myJettonWallet);
        ...
        b_1.storeCoins(src.balanceOld);
        b_1.storeInt(src.maxItemIndex, 257);
        b_1.storeRef(src.nftItemCode);
        b_0.storeRef(b_1.endCell());
    };
}

export function loadExchanger$Data(slice: Slice) {
    const sc_0 = slice;
    const _initialized = sc_0.loadBit();
    const _myJettonWallet = sc_0.loadMaybeAddress();
    const _myJettonWalletOld = sc_0.loadMaybeAddress();
    ...
    const _balanceOld = sc_1.loadCoins();
    const _maxItemIndex = sc_1.loadIntBig(257);
    const _nftItemCode = sc_1.loadRef();
    return { $$type: 'Exchanger$Data' as const, initialized: _initialized, myJettonWallet: _myJettonWallet, myJettonWalletOld: _myJettonWalletOld, collection: _collection, collectionOld: _collectionOld, balance: _balance, balanceOld: _balanceOld, maxItemIndex: _maxItemIndex, nftItemCode: _nftItemCode };
}

function loadTupleExchanger$Data(source: TupleReader) {
    const _initialized = source.readBoolean();
    const _myJettonWallet = source.readAddressOpt();
    const _myJettonWalletOld = source.readAddressOpt();
    const _collection = source.readAddress();
...
function loadGetterTupleExchanger$Data(source: TupleReader) {
    const _initialized = source.readBoolean();
    const _myJettonWallet = source.readAddressOpt();
    const _myJettonWalletOld = source.readAddressOpt();
    const _collection = source.readAddress();
...
function storeTupleExchanger$Data(source: Exchanger$Data) {
    const builder = new TupleBuilder();
    builder.writeBoolean(source.initialized);
    builder.writeAddress(source.myJettonWallet);
    builder.writeAddress(source.myJettonWalletOld);
...
function dictValueParserExchanger$Data(): DictionaryValue<Exchanger$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeExchanger$Data(src)).endCell());
        },
        parse: (src) => {
            return loadExchanger$Data(src.loadRef().beginParse());
        }
    }
}

 type Exchanger_init_args = {
    $$type: 'Exchanger_init_args';
    id: bigint;
    collection: Address;
    collectionOld: Address;
...
function initExchanger_init_args(src: Exchanger_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeInt(src.id, 257);
        b_0.storeAddress(src.collection);
        b_0.storeAddress(src.collectionOld);
        const b_1 = new Builder();
        b_1.storeInt(src.maxItemIndex, 257);
        b_1.storeRef(src.nftItemCode);
        b_0.storeRef(b_1.endCell());
...
async function Exchanger_init(id: bigint, collection: Address, collectionOld: Address, maxItemIndex: bigint, nftItemCode: Cell) {
    const __code = Cell.fromHex('');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initExchanger_init_args({ $$type: 'Exchanger_init_args', id, collection, collectionOld, maxItemIndex, nftItemCode })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

export const Exchanger_errors = {
    ...
    27881: { message: `Exchanger can be initialized only once` },
    32028: { message: `Item address mismatch` },
    37618: { message: `Notification not from ORBC wallet!` },
} as const

export const Exchanger_errors_backward = {
    "Stack underflow": 2,
    "Stack overflow": 3,
    "Integer overflow": 4,
    ...
    "Exchanger can be initialized only once": 27881,
    "Item address mismatch": 32028,
    "Notification not from ORBC wallet!": 37618,
} as const

const Exchanger_types: ABIType[] = [
    ...
    {"name":"ExchangerData","header":null,"fields":[{"name":"myJettonWallet","type":{"kind":"simple","type":"address","optional":true}},{"name":"myJettonWalletOld","type":{"kind":"simple","type":"address","optional":true}},{"name":"collection","type":{"kind":"simple","type":"address","optional":false}},{"name":"collectionOld","type":{"kind":"simple","type":"address","optional":false}},{"name":"balance","type":{"kind":"simple","type":"uint","optional":true,"format":"coins"}},{"name":"balanceOld","type":{"kind":"simple","type":"uint","optional":true,"format":"coins"}},{"name":"maxItemIndex","type":{"kind":"simple","type":"int","optional":false,"format":257}},{"name":"expectedNftItem0Address","type":{"kind":"simple","type":"address","optional":false}}]},
    {"name":"Exchanger$Data","header":null,"fields":[{"name":"initialized","type":{"kind":"simple","type":"bool","optional":false}},{"name":"myJettonWallet","type":{"kind":"simple","type":"address","optional":true}},{"name":"myJettonWalletOld","type":{"kind":"simple","type":"address","optional":true}},{"name":"collection","type":{"kind":"simple","type":"address","optional":false}},{"name":"collectionOld","type":{"kind":"simple","type":"address","optional":false}},{"name":"balance","type":{"kind":"simple","type":"uint","optional":false,"format":"coins"}},{"name":"balanceOld","type":{"kind":"simple","type":"uint","optional":false,"format":"coins"}},{"name":"maxItemIndex","type":{"kind":"simple","type":"int","optional":false,"format":257}},{"name":"nftItemCode","type":{"kind":"simple","type":"cell","optional":false}}]},
]

const Exchanger_opcodes = {
    "Excesses": 3576854235,
...
const Exchanger_getters: ABIGetter[] = [
    {"name":"get_exchanger_data","methodId":69016,"arguments":[],"returnType":{"kind":"simple","type":"ExchangerData","optional":false}},
]

export const Exchanger_getterMapping: { [key: string]: string } = {
    'get_exchanger_data': 'getGetExchangerData',
}

const Exchanger_receivers: ABIReceiver[] = [
    {"receiver":"internal","message":{"kind":"empty"}},
    {"receiver":"internal","message":{"kind":"typed","type":"InitExchangerJettonWallets"}},
...
export class Exchanger implements Contract {
    
    public static readonly storageReserve = 0n;
    public static readonly errors = Exchanger_errors_backward;
    public static readonly opcodes = Exchanger_opcodes;
    
    static async init(id: bigint, collection: Address, collectionOld: Address, maxItemIndex: bigint, nftItemCode: Cell) {
        return await Exchanger_init(id, collection, collectionOld, maxItemIndex, nftItemCode);
    }
    
    static async fromInit(id: bigint, collection: Address, collectionOld: Address, maxItemIndex: bigint, nftItemCode: Cell) {
        const __gen_init = await Exchanger_init(id, collection, collectionOld, maxItemIndex, nftItemCode);
        const address = contractAddress(0, __gen_init);
        return new Exchanger(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new Exchanger(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  Exchanger_types,
        getters: Exchanger_getters,
        receivers: Exchanger_receivers,
        errors: Exchanger_errors,
    };
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: null | InitExchangerJettonWallets | JettonTransferNotification | OwnershipAssigned) {
        
        let body: Cell | null = null;
        if (message === null) {
            body = new Cell();
        }
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'InitExchangerJettonWallets') {
            body = beginCell().store(storeInitExchangerJettonWallets(message)).endCell();
        }
    ...
    async getGetExchangerData(provider: ContractProvider) {
        const builder = new TupleBuilder();
        const source = (await provider.get('get_exchanger_data', builder.build())).stack;
        const result = loadGetterTupleExchangerData(source);
        return result;
    }
